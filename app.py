...
def load_excel(filename):
    ...
    try:
        ...
        return [
            dict(zip(headers, r)) for r in rows[1:] if any(v is not None for v in r)
        ]
...
def build_P(products, behavior_raw):
    ...
    categories = sorted(
        {str(p.get("category", "Unknown")).strip() for p in products.values()}
    )
...
def build_sample_data():
    ...
    users = {
        i: {"user_id": i, "age": 18 + i * 2, "location": f"City-{i}"}
        for i in range(1, 11)
    }
...
def init_population(Np, n_users, n_tags):
    ...
    """
    return [
        [[random.random() for _ in range(n_tags)] for _ in range(n_users)]
        for _ in range(Np)
    ]
...
def mutate(chrom, Pm):
    ...
    """
    return [
        [random.random() if random.random() < Pm else g for g in row] for row in chrom
    ]
...
def run_endpoint():
    ...
    # Run GA
    best_Q, history = run_ga(P, R, user_ids, Np=Np, Ni=Ni, Pc=Pc, Pm=Pm, seed=seed)
    ...
    if uid in uid_idx:
        ...
        q_values = [
            {"tag": t, "affinity": round(v, 4)} for t, v in zip(tag_names, q_row)
        ]
